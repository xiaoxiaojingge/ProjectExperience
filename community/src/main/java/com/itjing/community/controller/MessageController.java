...
public class MessageController implements CommunityConstant {
    ...
    private User getLetterTarget(String conversationId) {
        ...
        // 如果 id0 为当前登录用户 则表示是我私信对方,查出对方是谁
     ...
     * @return
     */
    @RequestMapping(path = "/notice/remove", method = RequestMethod.POST)
    @ResponseBody
    public String removeMessage(int noticeId) {
        int result = messageService.removeMessage(Arrays.asList(noticeId));
        if (result == 0) {
            return CommunityUtil.getJSONString(1, "remove fail!");
        }
        return CommunityUtil.getJSONString(0);
    }
}
